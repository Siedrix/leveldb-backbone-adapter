...
Adapter.ModelOverwrite = function (Model) {
	...
	Model.findOne = function (queryObject, callback) {
		var Self = this,
			models = [];
		...
		readStream.on('data', function(data){
			...
			_.each(queryObject, function (value, key) {
				if( !(data.value[key] && data.value[key] === value) ){
					pass = false;
				}
			});
		...
		.on('end', function(){
			if(models.length === 1){
				...
				deferred.reject({error: 'to many models in findOne'});
				if(callback) callback({error: 'to many models in findOne'}, null);
			}else{
