	...
	Backbone.sync = function (method, entity, options) {
		...
		if(entity.isCollection){
			...
			if(method === 'create'){
				...
				dbPut(model.get('id'), modelAsJSON)
				.then(options.success, options.error);

			}else if(method === 'delete'){
				//implement

				dbDel(model.get('id')).then(options.success, options.error);
			}
		}
...
Adapter.CollectionOverwrite = function (Collection) {
	...
	Collection.findOne = function(queryObject, callback){
		var q = this.prototype.fetchOne(queryObject, callback);

		q.done(function(model){
			callback(null, model);
		});
	};
...
Adapter.ModelOverwrite = function (Model) {
	...
	Model.get = function (id, callback) {
		...
		promise = _get(id).then(function(data) {
			data.id = id;
